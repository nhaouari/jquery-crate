...
export class Event extends EventEmitter {
  ...
  removeFromString(StringtoBeremove, s) {
    ...
    if (indexOfLast >= 0) {
      s =
        s.slice(0, indexOfLast) + s.slice(indexOfLast + StringtoBeremove.length)
    }
    return s
  }
  ...
  sendLocalBroadcast(msg) {
    ...
    this._communicationChannel.broadcast._source
      .getNeighbours()
      .forEach(neighbourId =>
  ...
  passMsgByBroadcast(elem, originID) {
    ...
    const message = this.getBroadcastMessageFormat(
      broadcast._protocol,
      causalId,
      isReady,
      packet
  ...
  sendBroadcast(msg, isReady = null) {
    ...
    this._document.lastSentMsgId = this._communicationChannel.broadcast.send(
      { ...msg, isReady },
      null,
      isReady,
      false

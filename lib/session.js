...
class session {
  ...
  /**
   * initialize connection to signling server to get ICEs 
  ...
  init() {
    request({
      ...
      this._options.webRTCOptions = (addresses && addresses.d) || {
        iceServers: [{
  ...
  justDoIt() {
    ...
    if (this._options.editingSession) {
      ...
      if (store.get("CRATE2-" + this._options.editingSession)) {
        this._options.signalingOptions = {};
        this._options.importFromJSON = store.get("CRATE2-" + this._options.editingSession);
    ...
    // WEBRTC
    const webRTCOptions = (this._options && this._options.webRTCOptions) ||
      (this._options && this._options.importFromJSON &&
    ...
    this._options.name = (this._options && this._options.name) ||
      (this._options && this._options.importFromJSON &&
  ...
  moveToNext() {
    if (this._next != null) {
      ...
      this.goTo(this.constructor.actualSession._options.signalingOptions.session)
    }
  ...
  goTo(sessionId) {
    ...
    this.constructor.focusOnSession(sessionId, s._options.signalingOptions.session)
  }
  ...
  static getCrateSession(id) {
    ...
    while (!found && search !== null) {
      let sessionId = search._options.signalingOptions.session
      if (id === sessionId) {
        return search
      }
      search = search._next
    ...
    for (var link of links) {
      if (link.href.includes(window.location.href.split('?')[0])) {
        link.onclick = function() {
          ...
          const s = session.getCrateSession(sessionId)
          if (s._previous) {
          ...
          if (jQuery(`#container-${sessionId}`).length) {
            ...
            var options= JSON.parse(JSON.stringify(self.config));
            options.editingSession = this.href.split('?')[1]
            var sess=new session(options)
          }

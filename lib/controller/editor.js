                ...
                switch (oper){
                                    ...
                                    case "insert":
                                            ...
                                            if (value.image != undefined) {
                                                 ...
                                                 //data = JSON.stringify({type:'image',text:value,att:att});
...
model.core.on('remoteInsert', function(element, indexp){
        
      /*  console.group('Remot Insert');
        console.dir(element);
        console.log(" attindex ="+indexp);
        console.groupEnd();
        */
        index=indexp-1;
     ...
     //   debugger;
        if (index!==-1){
            switch (element.type){
                        case "formula": 
                            editor.insertEmbed(index,'formula',element.text.formula, 'silent');
                        
                        break;
                        case "image": 
                            editor.insertEmbed(index,'image',element.text.image, 'silent');
                  
                        break;
                        case "video": 
                            editor.insertEmbed(index,'video',element.text.video, 'silent');
                     
                        break;
                        case "char": 


                            editor.insertText(index,element.text,element.att, 'silent');
                            
                             if (element.text != "\n") {
                              editor.removeFormat(index,1,'silent');   
                              }
                             break;
                        }
            if (element.att) {
                            if (element.text != "\n") {
                               editor.formatLine(index,element.att,'silent'); 
                               editor.formatText(index,1,element.att,'silent');         
                              } 
...
console.groupEnd();
  */    
        ...
        if (removedIndex !== -1){
            editor.deleteText(removedIndex, 1,'silent');
...
model.core.on('remoteCaretMoved', function(range, origin){
    ...
    if (!origin) return;
        ...
        if (self.markers[origin])  {
            self.markers[origin].update(range,true); // to keep avatar
            
        } else {  // to creat the avatar
            //self.markers.push(origin);
            self.markers[origin]= new Marker(origin,5*1000,range,editor.getModule('cursors'),false);
...
model.core.on('ping', function(origin,pseudo){
        if (self.markers[origin])  {
            ...
            //self.markers.push(origin);
            self.markers[origin]= new Marker(origin,5*1000,{index:0, length:0},editor.getModule('cursors'),false,false);
            self.markers[origin].setPseudo(pseudo);

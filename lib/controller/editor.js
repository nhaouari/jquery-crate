...
editor2.on('text-change', function(delta, oldDelta, source) {
    ...
    var cursors = editor2.getModule('cursors');
        ...
        if (cursors.cursors[origin]) {
            ...
            //var pseudo = 'Anonymous '+capitalize(animals.words[hash(this.origin)%animals.words.length]);
            //var color = getColor(origin).toString(); 
            cursors.setCursor(origin, range, "Anonymous ", "red");
        }
      /*  if (!origin) return;

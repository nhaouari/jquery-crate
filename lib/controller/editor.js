...
//
    ...
    if (store.get('myId')){
    markers[id].setPseudo(store.get('myId').pseudo);
    } 
...
model.core.on('ping', function(origin,pseudo){
        ...
        if (markers[origin])  {
            ...
            markers[origin].setPseudo(pseudo);
                  ...
                  if(op.attributes) {
                        if(op.attributes.commentAuthor){
                           var id = op.attributes.commentAuthor;
                            //
                            addCommentToList(op.attributes.comment,id,markers[id].animal,markers[id].pseudoName,markers[id].colorRGB, op.attributes.commentTimestamp);
                        }

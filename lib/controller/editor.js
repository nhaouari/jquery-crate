...
function EditorController(model, viewEditor){
 ...
 editor2.on('selection-change', function(range, oldRange, source) {
  if (range){
...
editor2.on('text-change', function(delta, oldDelta, source) {
    ...
    if (!origin) return;
        ...
        var cursors = editor2.getModule('cursors');
        
        window.crate_range2 = range; 

        if (cursors.cursors[origin]) {
            cursors.moveCursor(origin, range)   
        }else {
            var pseudo = 'Anonymous '+capitalize(animals.words[hash(origin)%animals.words.length]);
            var color = "rgb("+getColor(origin).toString()+")";

            window.crate_pseudo = pseudo;
            window.crate_color = color;

...
window.Marker = require('../view/marker.js')
/**
...
class EditorController {
  ...
  loadDocument(sessionID) {
    ...
    Marker.editorContainerID = this._editorContainerID
    ...
    if (store.get("CRATE2-" + sessionID)) {
      ...
      openIn();// this is to convert the links to inside links
    }
    ...
    this.markers[id] = new Marker(id, 5 * 1000, {
      index: 0,
      length: 0
    }, this.viewEditor.getModule('cursors'), false, true,this._editorContainerID)
  ...
  remoteInsert(element, indexp) {
    ...
    openIn() //optimize this command more
    this.cleanQuill()
  ...
  remoteCaretMoved(range, origin) {
    ...
    if (this.markers[origin]) {
      ...
      this.markers[origin] = new Marker(origin, 5 * 1000, range, this.viewEditor.getModule('cursors'), false,this._editorContainerID)
  ...
  atPing(origin, pseudo) {
    if (this.markers[origin]) {
      ...
      this.markers[origin] = new Marker(origin, 5 * 1000, {
        index: 0,
        length: 0
      }, this.viewEditor.getModule('cursors'), false, false,this._editorContainerID)
      this.markers[origin].setPseudo(pseudo)
    }
  }
